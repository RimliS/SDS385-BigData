...
##################################################################################################
#calculate prediction error for training dataset
...
#calculate prediction error for test dataset
lasso_pred_y = predict(lasso_cv, newx =as.matrix(test[,-65]), s = lasso_cv$lambda.min)

##MSE:
meansq_test <- mse(test$Y, lasso_pred_y)
##################################################################################################
...
for (j in 1:no_of_folds)
{
  ...
  tmp_lasso        = glmnet(x = as.matrix(left_in_data[,-65]), y = left_in_data$Y, 
                            alpha = 1)     
  tmp_lasso_y      = predict(tmp_lasso, newx = as.matrix(left_out_data[,-65]),
                             s = tmp_lasso$lambda.min) 
  tmp_mse_lasso[j] = mse(left_out_data$Y, tmp_lasso_y) 
  
}


result <- mean(tmp_mse_lasso)

#######################################################################################################
##Prediction error for out of sample:
lasso_y_hat <- predict(tmp_lasso, newx =as.matrix(test[,-65]), s = tmp_lasso$lambda.min)
meansq_test <- mse(test$Y, lasso_y_hat)
#######################################################################################################

##Plot of CV estimate of Moose error for different lambda:
...
#plot(log(lambda),sqrt(moose_error),ty='h',ylab="sqrt of MSE")
